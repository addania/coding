...
<Layout>
  ...
  <p style={{textAlign: "center"}}>infiltrating the dev universe in order to <s>conquer it</s> learn a lot ;)</p>
  <Carousel />
          ...
          <tbody> 
            {data.allMarkdownRemark.edges.map(({ node }, index) => (
              <tr key={index}>
                ...
                <td>{limitLength(removeDateFromExcerpt(removeImageTextFromExcerpt(node.excerpt)))}</td>
...
query {
  allMarkdownRemark (sort: {fields:[frontmatter___date], order:DESC},filter: {frontmatter: {category: {eq: "impressions"}}}) {
    edges {
      node {
        id
        excerpt
